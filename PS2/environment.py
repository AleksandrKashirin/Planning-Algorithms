...
class State:  # Класс состояние

    def __init__(self, angles: np.ndarray):  # Определение экземпляра класса
        """
        Represents the state of the 4-link manipulator.  # Представляет состояние 4-звенного манипулятора в пространстве состояний

        :param angles: 4 angles for each link of the manipulator in degrees. Shape: (4,).  # 4 угла для каждой степени свободы в градусах
        """
        assert angles.shape == (4,)  # Проверяем формат ввода углов
...
class ManipulatorEnv:
    ...
    def render(self, plt_title, plt_show=True) -> None:
        """
